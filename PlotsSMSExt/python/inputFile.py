...
class inputFile():
    ...
    def __init__(self, fileName):
        ...
        self.LIMIT1 = self.findLIMIT(fileName, "1STLIMIT")
        self.LIMIT2 = self.findLIMIT(fileName, "2NDLIMIT")
    ...
    def findLIMIT(self, fileName, whichlimit):
        fileIN = open(fileName)        
        for line in fileIN:
            tmpLINE =  line[:-1].split(" ")
            if tmpLINE[0] != whichlimit: continue
            fileIN.close()
            rootFileIn = rt.TFile.Open(tmpLINE[1])
