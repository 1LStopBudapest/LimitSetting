...
if isSignal:
    for p in signals:
        ...
        f = ROOT.TFile.Open('File/PromptBK'+reg+'_SR+CR_T2tt_'+p+'.root')
        h = f.Get('h_reg_'+proc)
        ...
        fjec = ROOT.TFile.Open('File/PromptBK'+reg+'JEC_SR+CR_T2tt_'+p+'.root')
        hNom = fjec.Get('h_reg_'+proc)
    ...
    rate = []
    f = ROOT.TFile.Open('File/PromptBK'+reg+'_SR+CR_'+sname[proc]+'.root')
    ...
    fjec = ROOT.TFile.Open('File/PromptBK'+reg+'JEC_SR+CR_'+sname[proc]+'.root')
    hNom = fjec.Get('h_reg_'+sname[proc])
