...
if BKmode == 'old':
    ...
    c.SaveAs("PromptBK"+reg+"_postfitplot_SR+CR.png")
    c.Close()
    ...
    hWJets = ROOT.TH1F('hWJets', 'hWJets', bins, 0, bins)
    httbar = ROOT.TH1F('httbar', 'ttbar', bins, 0, bins)
    hOtherPrompt = ROOT.TH1F('hOtherPrompt', 'hOtherPrompt', bins, 0, bins)
    hFake = ROOT.TH1F('hFake', 'hFake', bins, 0, bins)
    hdata = ROOT.TH1F('hdata', 'hdata', bins, 0, bins)
    for b in range(bins):
        hWJets.GetXaxis().SetBinLabel(b+1, binLabel[b])
        httbar.GetXaxis().SetBinLabel(b+1, binLabel[b])
        hOtherPrompt.GetXaxis().SetBinLabel(b+1, binLabel[b])
        hFake.GetXaxis().SetBinLabel(b+1, binLabel[b])
        hdata.GetXaxis().SetBinLabel(b+1, binLabel[b])
    ...
    for b in range(bins):
        ...
        h_WJets = fin.Get(first_dir + "/" + second_dir + "/WJets")
        h_ttbar = fin.Get(first_dir + "/" + second_dir + "/ttbar")
        h_OtherPrompt = fin.Get(first_dir + "/" + second_dir + "/OtherPrompt")
        h_Fake = fin.Get(first_dir + "/" + second_dir + "/Fake")
        ...
        if h_ttbar:
            ...
            httbar.SetBinContent(b+1, 0)
        if h_OtherPrompt:
            hOtherPrompt.SetBinContent(b+1, h_OtherPrompt.GetBinContent(1))
            hOtherPrompt.SetBinError(b+1, h_OtherPrompt.GetBinError(1))
        else:
            hOtherPrompt.SetBinContent(b+1, 0)
        if h_Fake:
            hFake.SetBinContent(b+1, h_Fake.GetBinContent(1))
            hFake.SetBinError(b+1, h_Fake.GetBinError(1))
        else:
            hFake.SetBinContent(b+1, 0)
    ...
    hStack_MC = ROOT.THStack("hStack_MC","hStack_MC")
    hMC = hFake.Clone("TotalMC")
    hStack_MC.Add(hFake)
    hFake.SetFillColor(ROOT.kOrange-3)
    hFake.SetLineColor(ROOT.kOrange-3)
    legend.AddEntry(hFake, "Fake"+" ("+str(round(hFake.Integral(), 2))+")", "F")
    hStack_MC.Add(hOtherPrompt)
    hMC.Add(hOtherPrompt)
    hOtherPrompt.SetFillColor(ROOT.kMagenta+3)
    hOtherPrompt.SetLineColor(ROOT.kMagenta+3)
    legend.AddEntry(hOtherPrompt, "OtherPrompt"+" ("+str(round(hOtherPrompt.Integral(), 2))+")", "F")
    hStack_MC.Add(httbar)
    hMC.Add(httbar)
    httbar.SetFillColor(5)
    httbar.SetLineColor(5)
