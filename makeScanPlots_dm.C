...
void makeScanPlots_dm(const TString inputFileName = "results_T2tt.root", const TString outputFileName = "limit_scan_T2tt.root", bool expectedOnly = false, bool additionalInterpolation = false)
{
  ...
  vector<double> mstops, dms, mstopsxsec, dmsxsec;
  ...
  for(int ibinx = 1; ibinx <= hxsecexp->GetNbinsX()+1; ++ibinx) {
    for(int ibiny = 1; ibiny <= hxsecexp->GetNbinsY()+1; ++ibiny) {
      if(hxsecexp->GetBinContent(ibinx, ibiny) != 0.0) {
        ...
        dmsxsec.push_back(hxsecexp->GetYaxis()->GetBinLowEdge(ibiny));
  ...
  for(int ibinx = 1; ibinx <= hexp->GetNbinsX()+1; ++ibinx) {
    for(int ibiny = 1; ibiny <= hexp->GetNbinsY()+1; ++ibiny) {
      if(hexp->GetBinContent(ibinx, ibiny) != 0.0) {
        ...
        dms.push_back(hexp->GetYaxis()->GetBinLowEdge(ibiny));
        ...
        printf("MStop: %4.2f, MLSP: %4.2f, Limit: %4.2f, (+1: %4.2f, -1: %4.2f), Obs Limit: %4.2f (+1 theory: %4.2f, -1 theory: %4.2f)\n", mstops.back(), dms.back(), exp.back(), expup.back(), expdown.back(), obs.back(), obsup.back(), obsdown.back());
      }
    }
  }

  TGraph2D glimexp("glimexp", "Cross-Section Limit", expxsec.size(), &mstopsxsec.at(0), &dmsxsec.at(0), &expxsec.at(0));
  TGraph2D glimobs("glimobs", "Cross-Section Limit", obsxsec.size(), &mstopsxsec.at(0), &dmsxsec.at(0), &obsxsec.at(0));
  TGraph2D gexp("gexp", "Expected Limit", exp.size(), &mstops.at(0), &dms.at(0), &exp.at(0));
  TGraph2D gexpdown("gexpdown", "Expected  -1#sigma Limit", expdown.size(), &mstops.at(0), &dms.at(0), &expdown.at(0));
  TGraph2D gexpup("gexpup", "Expected  -1#sigma Limit", expup.size(), &mstops.at(0), &dms.at(0), &expup.at(0));
  TGraph2D gobs("gobs", "Observed Limit", obs.size(), &mstops.at(0), &dms.at(0), &obs.at(0));
  TGraph2D gobsdown("gobsdown", "Observed  -1#sigma (theory) Limit", obsdown.size(), &mstops.at(0), &dms.at(0), &obsdown.at(0));
  TGraph2D gobsup("gobsup", "Observed  -1#sigma (theory) Limit", obsup.size(), &mstops.at(0), &dms.at(0), &obsup.at(0));
  TGraph dots(mstops.size(), &mstops.at(0), &dms.at(0));

  double xmin = *min_element(mstops.cbegin(), mstops.cend());
  double xmax = *max_element(mstops.cbegin(), mstops.cend());
  double ymin = *min_element(dms.cbegin(), dms.cend());
  double ymax = *max_element(dms.cbegin(), dms.cend());
